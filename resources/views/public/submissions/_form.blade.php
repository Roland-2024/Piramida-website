...
@if ($formType === 'leasing')
    @include('public.submissions._leasing-form', ['action' => $action])
@else
<form method="POST" action="{{ $action }}" enctype="{{ $formType === 'career' ? 'multipart/form-data' : 'application/x-www-form-urlencoded' }}" class="mt-6 grid gap-5 sm:grid-cols-2">
    @csrf
    @if ($formType === 'career')
        <div><label for="request_first_name" class="block text-sm font-medium">{{ __('cms.first_name') }}</label><input id="request_first_name" name="first_name" value="{{ old('first_name') }}" required class="mt-2 block w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm">@error('first_name')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror</div>
        <div><label for="request_last_name" class="block text-sm font-medium">{{ __('cms.last_name') }}</label><input id="request_last_name" name="last_name" value="{{ old('last_name') }}" required class="mt-2 block w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm">@error('last_name')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror</div>
    @else
    ...
    <div><label for="request_phone" class="block text-sm font-medium">{{ __('cms.phone') }}</label><input id="request_phone" name="phone" value="{{ old('phone') }}" @required(in_array($formType, ['event', 'event_space', 'career'], true)) class="mt-2 block w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm">@error('phone')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror</div>

    @if ($formType === 'event_space')
