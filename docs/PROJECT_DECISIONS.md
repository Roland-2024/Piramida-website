...
- About uses the English slug `about-us` and the seeded Overview, Mission, History and Timeline section names. Education, Innovation, Business and Art share the revised presentation layout, mapped by English slugs `education`, `innovation`, `business`, `art`. Each active Text and Image / Gallery section supplies ordered slides; a section title/subtitle and description supply its caption, while multi-image galleries use each image’s localized alt text as its title. Other pages keep the generic renderer. Preserve these identifiers until a final template mapping is agreed.
- The Business presentation page is separate from the Businesses directory and its experience popups. Missing presentation pages can be added with `PresentationPageSeeder`; its shared-template placeholder slides are editable in Page Sections. It skips all existing pages, including drafts and soft-deleted records.
